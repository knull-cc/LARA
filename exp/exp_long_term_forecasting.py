...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def _maybe_prepare_lara_memory(self, train_data):
        ...
        if getattr(model, 'supports_lara_context', False) and not getattr(model, 'memory_prepared', False):
            model.prepare_memory(train_data, device=self.device)

    def _unpack_batch(self, batch):
        if len(batch) == 5:
            batch_x, batch_y, batch_x_mark, batch_y_mark, sample_index = batch
        else:
            batch_x, batch_y, batch_x_mark, batch_y_mark = batch
            sample_index = None
        return batch_x, batch_y, batch_x_mark, batch_y_mark, sample_index

    def _forward_model(self, batch_x, batch_x_mark, dec_inp, batch_y_mark,
                       sample_index=None, mode='train', y_true=None):
        if self._is_lara_model():
            return self.model(
                batch_x,
                batch_x_mark,
                dec_inp,
    ...
    def _add_lara_aux_loss(self, loss):
        if not self._is_lara_model():
            return loss
        aux_loss = getattr(self._model_ref(), 'aux_loss', None)
        if aux_loss is None:
            return loss
        return loss + aux_loss

    def _print_lara_diagnostics(self):
        if not self._is_lara_model():
            return
        diagnostics = self._model_ref().get_lara_diagnostics(reset=True)
        if not diagnostics:
            return
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        with torch.no_grad():
            for i, batch in enumerate(vali_loader):
                batch_x, batch_y, batch_x_mark, batch_y_mark, sample_index = self._unpack_batch(batch)
    ...
    def train(self, setting):
        ...
        self._maybe_prepare_lara_memory(train_data)
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, batch in enumerate(train_loader):
                ...
                batch_x_mark = batch_x_mark.float().to(self.device)
                batch_y_mark = batch_y_mark.float().to(self.device)
                target_for_model = batch_y[:, -self.args.pred_len:, :]
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        outputs = self._forward_model(
                            batch_x, batch_x_mark, dec_inp, batch_y_mark,
                            sample_index=sample_index, mode='train',
                            y_true=target_for_model)

                        f_dim = -1 if self.args.features == 'MS' else 0
                        outputs = outputs[:, -self.args.pred_len:, f_dim:]
                        batch_y = batch_y[:, -self.args.pred_len:, f_dim:].to(self.device)
                        loss = criterion(outputs, batch_y)
                        loss = self._add_lara_aux_loss(loss)
    ...
    def test(self, setting, test=0):
        if self._is_lara_model() and not getattr(self._model_ref(), 'memory_prepared', False):
